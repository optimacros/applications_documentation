...
* `items` — пока что не используется.
...
periods(): ModelItem[];
```
Возвращает массив временных периодов (включая пользовательские). Элементы массива отличаются от указанного в сигнатуре типа [`ModelItem`](#model-item) тем, что в них отсутствует поле `code`.
...
subsets(): ModelItems;
```
Возвращает объект [`ModelItems`](#model-items) с массивом всех выборок версий модели. При этом выборки не связаны с версиями.
